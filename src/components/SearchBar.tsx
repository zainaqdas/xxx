    ...
    <form onSubmit={handleSubmit} className="space-y-3">
      <div className="flex gap-2">
        <input
          ...
          className="flex-1 min-w-0 px-4 py-3 bg-gray-800 border border-gray-700 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-red-500 focus:ring-1 focus:ring-red-500 transition-colors text-sm"
        />
        <button
          type="submit"
          className="px-4 py-3 bg-red-600 hover:bg-red-700 text-white rounded-xl font-medium transition-colors flex items-center gap-2"
        >
      ...
      {/* Filters - 2 columns on mobile, row on desktop */}
      <div className="grid grid-cols-2 sm:flex sm:flex-wrap gap-2">
        ...
        <select value={durf} onChange={(e) => setDurf(e.target.value)} className="px-3 py-2 bg-gray-800 border border-gray-700 rounded-lg text-sm text-gray-300 focus:outline-none focus:border-red-500">
          <option value="allduration">Duration</option>
